  ...
  try {
    ...
    if (req.body.certificate) {
      let array = await course.find({
        courseSpecification: req.body.certificate,
      });
      if (array.length > 0) {
        finalArray.push(array);
      }
    ...
    if (req.body.nonCertificate) {
      let array = await course.find({
        courseSpecification: req.body.nonCertificate,
      });
      if (array.length > 0) {
        finalArray.push(array);
      }
    ...
    if (req.body.certificateCredit) {
      let array = await course.find({
        courseSpecification: req.body.certificateCredit,
      });
      if (array.length > 0) {
        finalArray.push(array);
      }
    ...
    if (req.body.language) {
      let array = await course.find({ courseLanguage: req.body.language });
    ...
    if (req.body.country) {
      let array = await course.find({ courseCountry: req.body.country });
      if (array.length > 0) {
        finalArray.push(array);
      }
    }
